      ...
      <div style={{ background: "var(--s1)", border: "1px solid var(--b2)", borderRadius: 16, padding: 24, width: 360, position: "relative", maxWidth: "95vw" }}>
        ...
        {step === "choose" && (
          <>
            ...
            <div style={{ fontSize: 12, color: "var(--mu)", marginBottom: 20, lineHeight: 1.5 }}>
              Use a Solana wallet or sign in with your email.
            </div>

            {error && (
              <div style={{ padding: "10px 12px", borderRadius: 8, background: "#ff4d6d15", border: "1px solid #ff4d6d40", color: "#ff4d6d", fontSize: 12, marginBottom: 12, lineHeight: 1.5 }}>
                ⚠ {error}
              </div>
            )}

            {/* Email option */}
            <div onClick={() => { setError(null); setStep("email"); }} style={{ display: "flex", alignItems: "center", gap: 12, padding: 13, borderRadius: 10, border: "1px solid var(--ac)", background: "#00e5a008", cursor: "pointer", marginBottom: 10 }}>
            ...
            {WALLETS.map((w) => (
              <div key={w.id} onClick={() => handleWallet(w.id)}
                style={{ display: "flex", alignItems: "center", gap: 12, padding: 12, borderRadius: 9, border: "1px solid var(--b2)", background: "var(--s2)", cursor: "pointer", marginBottom: 7, opacity: connecting && connecting !== w.id ? .5 : 1 }}>
                ...
                {connecting === w.id
                  ? <div style={{ marginLeft: "auto", fontSize: 11, color: "var(--a4)" }}>Connecting…</div>
                  : <div style={{ marginLeft: "auto", fontSize: 10, color: "var(--mu)" }}>→</div>
                }
              </div>
            ))}
            ...
            <div style={{ marginTop: 14, fontSize: 10, color: "var(--mu)", lineHeight: 1.6, textAlign: "center" }}>
              Don't have a wallet?{" "}
              <span onClick={() => window.open("https://phantom.app", "_blank")} style={{ color: "var(--a3)", cursor: "pointer" }}>Get Phantom</span>
              {" "}or{" "}
              <span onClick={() => window.open("https://solflare.com", "_blank")} style={{ color: "var(--a3)", cursor: "pointer" }}>Get Solflare</span>
            </div>
          </>
        )}
        ...
        {step === "otp" && (
          <>
            ...
            />
            <button onClick={handleOtpVerify} disabled={otp.length < 6 || !!connecting}
              style={{ width: "100%", padding: 11, borderRadius: 8, border: "none", background: connecting ? "var(--a4)" : "var(--ac)", color: "#000", fontWeight: 800, fontSize: 13, cursor: otp.length >= 6 ? "pointer" : "default", fontFamily: "var(--ff)", opacity: otp.length < 6 ? .5 : 1 }}>
